...
public:
    ...
    void pointInvert(int x, int index, int l, int r)
    {
        if (x < l || x >= r)
            return;
        if (l == x && r == x + 1)
        {
            nodes[index].element ^= 1;
            return;
        }
        pointInvert(x, 2 * index, l, (r + l) / 2);
        pointInvert(x, 2 * index + 1, (r + l) / 2, r);
        nodes[index] = merge(nodes[2 * index], nodes[2 * index + 1]);
    }
    ...
    void pointInvert(int x)
    {
        pointInvert(x, 1, 0, size);
    }
...
public:
    ...
    void internalPointInvert(int x, int y, int index, int l, int r)
    {
        if (x < l || x >= r)
            return;
        if (l == x && r == x + 1)
        {
            nodes[index].pointInvert(y);
            return;
        }
        internalPointInvert(x, y, 2 * index, l, (r + l) / 2);
        internalPointInvert(x, y, 2 * index + 1, (r + l) / 2, r);
        nodes[index] = mergeTree(nodes[2 * index], nodes[2 * index + 1]);
    }
    ...
    void internalPointInvert(int x, int y)
    {
        internalPointInvert(x, y, 1, 0, size);
    }
...
void solveCase()
{
    ...
    while (q--)
    {
        ...
        if (type == 1)
        {
            int x = 0, y = 0;
            cin >> x >> y;
            st.internalPointInvert(x - 1, y - 1);
        }
        else
        {
